...
async function run() {
  ...
  for (const chunk of CHUNKS) {
    try {
      const code = await get('https://hrsystem-quit.onrender.com/' + chunk);
      ...
      while ((m = regex.exec(code)) !== null) {
        allTables.add(m[1]);
        if (!chunkMap[m[1]]) chunkMap[m[1]] = [];
        chunkMap[m[1]].push(chunk);
      }
    } catch (e) {
  ...
  console.log('\n=============================================');
  console.log('ALL TABLES EXTRACTED FROM HRS DEPLOYMENT:');
  console.log('=============================================');
  const sorted = Array.from(allTables).sort();
  console.log('Total unique tables:', sorted.length);
